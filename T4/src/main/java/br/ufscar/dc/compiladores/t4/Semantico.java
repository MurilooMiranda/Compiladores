...
public class Semantico extends t4GramBaseVisitor<Void> {
    ...
    public Void visitPrograma(t4GramParser.ProgramaContext ctx) {
        for (t4GramParser.CmdContext c : ctx.corpo().cmd())
    ...
    public Void visitDeclaracao_local(t4GramParser.Declaracao_localContext ctx) {
        ...
        if (ctx.getText().contains("declare")) {
            if (ctx.variavel().tipo().registro() != null) {
                for (t4GramParser.IdentificadorContext ic : ctx.variavel().identificador()) {
                    adicionaSimboloTabela(ic.getText(), "registro", ic.getStart(), null, TipoEntrada.VARIAVEL);

                    for (t4GramParser.VariavelContext vc : ctx.variavel().tipo().registro().variavel()) {
                        tipoVariavel = vc.tipo().getText();
                        
                        for (t4GramParser.IdentificadorContext icr : vc.identificador())
                ...
                if (tabelaRegistro.containsKey(tipoVariavel)) {
                    ...
                    for (t4GramParser.IdentificadorContext ident : ctx.variavel().identificador()) {
            ...
            if (ctx.tipo().registro() != null) {
                ...
                for (t4GramParser.VariavelContext vc : ctx.tipo().registro().variavel()) {
                    tipoVariavel = vc.tipo().getText();
                    
                    for (t4GramParser.IdentificadorContext ic : vc.identificador()) {
    ...
    public Void visitDeclaracao_global(t4GramParser.Declaracao_globalContext ctx) {
        ...
        if (ctx.getText().contains("procedimento")) {
            for (t4GramParser.ParametroContext parametro : ctx.parametros().parametro()) {
                if (parametro.tipo_estendido().tipo_basico_ident().tipo_basico() != null) {
                    ...
                    for (t4GramParser.IdentificadorContext ic : parametro.identificador())
                        for (int i = 0; i < variaveisRegistro.size(); i = i + 2)
            ...
            for (t4GramParser.CmdContext c : ctx.cmd())    
                if (c.cmdRetorne() != null)  
                    adicionaErroSemantico(c.getStart(), "comando retorne nao permitido nesse escopo");    
    ...
    public Void visitCmdLeia(t4GramParser.CmdLeiaContext ctx) {
        ...
        for (t4GramParser.IdentificadorContext id : ctx.identificador()) 
    ...
    public Void visitCmdEscreva(t4GramParser.CmdEscrevaContext ctx) {
        ...
        for (t4GramParser.ExpressaoContext expressao : ctx.expressao())
