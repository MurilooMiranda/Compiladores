...
import static br.ufscar.dc.compiladores.t3.T3SemanticoUtils.verificarTipo;
import static br.ufscar.dc.compiladores.t3.T3SemanticoUtils.adicionaErroSemantico;
import static br.ufscar.dc.compiladores.t3.T3SemanticoUtils.verificaCompatibilidade;
...
public class T3Semantico extends t3GramBaseVisitor<Void> {
    ...
    @Override
    public Void visitPrograma(t3GramParser.ProgramaContext ctx) {
    ...
    @Override
    public Void visitDeclaracoes(t3GramParser.DeclaracoesContext ctx) {
        tabela = escoposAninhados.obterEscopoAtual();
        
        // Verifica a declaração atual.
        for (t3GramParser.Decl_local_globalContext declaracao : ctx.decl_local_global())
    ...
    @Override
    public Void visitDecl_local_global(t3GramParser.Decl_local_globalContext ctx) {
        tabela = escoposAninhados.obterEscopoAtual();
    ...
    @Override
    public Void visitDeclaracao_local(t3GramParser.Declaracao_localContext ctx) {
        tabela = escoposAninhados.obterEscopoAtual();
        ...
        if (ctx.getText().contains("declare")) {
            ...
            for (t3GramParser.IdentificadorContext ident : ctx.variavel().identificador()) {
    ...
    @Override
    public Void visitCmdLeia(t3GramParser.CmdLeiaContext ctx) {
        tabela = escoposAninhados.obterEscopoAtual();

        for (t3GramParser.IdentificadorContext id : ctx.identificador())
    ...
    @Override
    public Void visitCmdEscreva(t3GramParser.CmdEscrevaContext ctx) {
        tabela = escoposAninhados.obterEscopoAtual();
        
        TipoT3 tipo;

        for (t3GramParser.ExpressaoContext expressao : ctx.expressao())
    ...
    @Override
    public Void visitCmdEnquanto(t3GramParser.CmdEnquantoContext ctx) {
        tabela = escoposAninhados.obterEscopoAtual();
    ...
    @Override
    public Void visitCmdAtribuicao(t3GramParser.CmdAtribuicaoContext ctx) {
        tabela = escoposAninhados.obterEscopoAtual();
